...
from app.models.models import utc_now, User
...
async def handle_bot_message(
    ...
    if text == "/start":
        ...
        notes = await note_repo.get_all_by_user_id(user.id)
        if not notes:
            response_text = "📝 Anda belum memiliki catatan tersimpan."
        else:
            lines = ["📝 **Catatan Terakhir Anda:**"]
            for note in notes[:5]:
                tags_str = ", ".join(note.tags) if note.tags else "-"
                lines.append(
                    f"- **{note.title}**: {note.summary or note.content[:50]} (Tag: {tags_str})"
                )
            response_text = "\n".join(lines)
        return BotMessageResponse(response_text=response_text)
        ...
        if not txs:
            response_text = "💵 Anda belum memiliki transaksi tersimpan."
        else:
            lines = ["💵 **Transaksi Terakhir Anda:**"]
            for tx in txs[:5]:
                lines.append(
...
@router.post("/chat", response_model=BotMessageResponse)
async def handle_web_chat(
    payload: WebChatInput,
    current_user: Annotated[User, Depends(get_current_user)],
    db: Annotated[AsyncSession, Depends(get_db)],
):
    """Handle chat messages sent from the React web dashboard, authorized by JWT."""
    text = payload.text.strip()

    # Handlers for static commands
    if text == "/start":
        response_text = (
            f"Halo {current_user.full_name or 'Pengguna'}! Selamat datang di Syntho.\n\n"
            "Saya adalah asisten AI pribadi Anda untuk mencatat pengeluaran dan catatan.\n"
            "Kirimkan pesan seperti:\n"
            "- 'Beli kopi 20 ribu'\n"
            "- 'Gaji masuk 3 juta'\n"
        ...
        note_repo = NoteRepository(db)
        notes = await note_repo.get_all_by_user_id(current_user.id)
        if not notes:
            response_text = "📝 Anda belum memiliki catatan tersimpan."
        else:
            lines = ["📝 **Catatan Terakhir Anda:**"]
        ...
        tx_repo = TransactionRepository(db)
        txs = await tx_repo.get_all_by_user_id(current_user.id)
        if not txs:
            response_text = "💵 Anda belum memiliki transaksi tersimpan."
        else:
            lines = ["💵 **Transaksi Terakhir Anda:**"]
    ...
    try:
        ...
        logger.exception("Failed to process web chat message using AI agent system.")
        response_text = (
            "⚠️ Mohon maaf, terjadi gangguan saat memproses pesan Anda dengan AI. "
            "Pesan Anda gagal dianalisis."
        )
